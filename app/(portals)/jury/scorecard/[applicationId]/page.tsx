...
export default async function JuryScorecardPage({ params }: { params: Promise<{ applicationId: string }> }) {
  ...
  const [cards, progress] = await Promise.all([
    getJurorScorecards(applicationId, user.id),
    getScorecardProgress(applicationId, user.id),
  ]);
    ...
    return (
      <div className="max-w-lg mx-auto px-6 py-16 text-center">
        ...
        <p className="text-sm text-text-muted mt-2">
          You submitted your pillar scorecard for {application.organization.name}. Scores are final once submitted.
        </p>
      </div>
    );
  }
