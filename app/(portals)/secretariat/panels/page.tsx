...
export default async function PanelsPage() {
  const [panels, sectors, unassignedSectorIds, conflicts, applications] = await Promise.all([
  ...
  const jurors = store.users.filter((u) => u.role === "jury");
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="h-17 border-b border-border flex items-center justify-between px-6 sm:px-7 flex-shrink-0">
        ...
        <ConflictOfInterestForm jurors={jurors} applications={applicationOptions} />
      </div>
          ...
          {panels.map((panel) => (
            <div key={panel.id} className="border border-border rounded-2xl p-5">
              ...
              <div className="text-xs text-text-muted mb-3.5">
                {panel.jurorNames.join(", ")} · {panel.shortlistedCount} shortlisted applicant
                {panel.shortlistedCount === 1 ? "" : "s"}
              </div>
        ...
        <div>
          ...
          {conflicts.length === 0 ? (
            ...
            <div className="border border-border rounded-2xl overflow-hidden">
              {conflicts.map((c) => {
                const juror = store.users.find((u) => u.id === c.jurorId);
                ...
                return (
                  <div key={c.id} className="px-4.5 py-3.5 border-b last:border-b-0 border-border text-[13px] flex justify-between gap-3">
                    <div>
                      <span className="font-semibold">{juror?.name}</span> — {c.reason}
