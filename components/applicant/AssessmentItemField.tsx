  ...
  return (
    <div className="border border-border rounded-2xl p-5">
      <div className="flex items-start justify-between gap-3 mb-3.5">
        ...
        <span className="flex-shrink-0 text-[10px] font-bold uppercase text-text-muted border border-border rounded-full px-2.5 py-1">
          {item.track}
        </span>
      </div>

      {!isNA && <ResponseControl item={item} value={value} onChange={(v) => onChange(v, false, "")} />}
...
function ResponseControl({
  ...
  switch (item.responseType) {
      ...
      return (
        <div className="flex flex-col gap-2">
          {item.maturityLabels!.map((label, idx) => {
            const level = idx + 1;
            return (
              <ChoiceRow key={level} active={value === level} onClick={() => onChange(level)}>
      ...
      return (
        <div className="grid grid-cols-5 gap-2">
          {FREQUENCY_LABELS.map((label, idx) => (
            <ChoiceButton key={label} active={value === idx} onClick={() => onChange(idx)}>
              {label}
            </ChoiceButton>
          ))}
        </div>
    ...
    case "narrative": {
      ...
      return (
        <div>
          <Textarea rows={4} placeholder="200–300 words" value={text} onChange={(e) => onChange(e.target.value)} />
...
function ChoiceButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex-1 rounded-xl border-[1.5px] px-3 py-2.5 text-[13px] font-semibold text-center",
...
function ChoiceRow({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "text-left rounded-xl border-[1.5px] px-3.5 py-2.5 text-[13px]",
