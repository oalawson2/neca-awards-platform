...
echo "==> [1/7] Pulling latest from git..."
git pull

echo "==> [2/7] Activating Node virtual environment..."
...
echo "==> [4/7] Checking build-time env vars are present..."
...
echo "==> [5/7] Building (RAYON_NUM_THREADS=1 — required on this host, see next.config.ts)..."
RAYON_NUM_THREADS=1 npm run build

echo "==> [6/7] Forcing standalone server to bind 0.0.0.0..."
# Next's own generated .next/standalone/server.js (freshly regenerated by
# every `next build`, so this has to be patched post-build, not edited
# once) contains `const hostname = process.env.HOSTNAME || '0.0.0.0'` —
# see node_modules/next/dist/build/utils.js's SERVER_FILE template. This
# host exports an ambient HOSTNAME shell var (its own name,
# "server224.web-hosting.com") that Passenger inherits when it spawns the
# Node app, so without this Next binds *only* to that resolved address
# instead of all interfaces — Passenger/LiteSpeed proxy to the app over
# localhost/a private socket, so an app that isn't listening on 0.0.0.0 is
# unreachable from them even though it runs fine and returns real 200s
# when hit directly by that exact hostname. Force it unconditionally so no
# ambient env var can narrow the bind again.
sed -i "s/const hostname = process.env.HOSTNAME || '0.0.0.0'/const hostname = '0.0.0.0'/" .next/standalone/server.js

echo "==> [7/7] Staging standalone output (public/ and .next/static aren't included by Next's standalone build)..."
